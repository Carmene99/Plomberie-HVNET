<?php
 ...
 if(!isset($_SESSION["login"])){
  ...
  $stmt = $conn->prepare("SELECT * from debouchage_wc LIMIT 1");
  $stmt->execute();
  $fetch = $stmt->fetch();
 ...
 /* update the imgs */
  if(isset($_POST["update_imgs"])){
    if($_FILES["img1"]["name"]=="" && $_FILES["img2"]["name"]==""){
        ...
        $stmt = $conn->prepare("UPDATE debouchage_wc SET img1=:img1");
        ...
        $stmt = $conn->prepare("UPDATE debouchage_wc SET img2=:img2");
        ...
        unlink("imgs/".$fetch["img1"]);
        unlink("imgs/".$fetch["img2"]);
        $stmt = $conn->prepare("UPDATE debouchage_wc SET img1=:img1,img2=:img2");
        $img1 = $_FILES["img1"]["name"];
        $img2 = $_FILES["img2"]["name"];
        $stmt->bindParam(":img1",$img1);
        $stmt->bindParam(":img2",$img2);
...
//   update the presentation
  if(isset($_POST["update_presentation"])){
         ...
         $stmt = $conn->prepare("UPDATE debouchage_wc SET presentation=:presentation");
